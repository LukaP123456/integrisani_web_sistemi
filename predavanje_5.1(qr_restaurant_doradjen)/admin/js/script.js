$(document).ready(function(){
    ...
    $('#menuCategories').DataTable({
        ajax: 'ajax/getMenuCategories.php',
    });

    $('body').on('click', '.deleteMenuCategories', function() {
        var id = $(this).data('id');
        let name = $(this).data('name');
        let answer = confirm('Do you want to delete category with name '+name+' ?');
        if(answer) {
            $.post('ajax/deleteMenuCategory.php', { id: id});
            $('#menuCategories').DataTable().ajax.reload();
            $('#menuCategories').DataTable().clear();
        }
    })
}); 
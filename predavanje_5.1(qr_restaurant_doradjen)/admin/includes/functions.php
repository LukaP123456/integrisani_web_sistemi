...
function insertIntoWaitersTable(string $name, string $password, mysqli $connection): void
{
    $password = password_hash($password, PASSWORD_BCRYPT);

    $sql = "INSERT INTO waiters(name, password) VALUES ('$name', '$password')";
    mysqli_query($connection, $sql) or die(mysqli_error($connection));
}
...
/**
 * Checks if data exists in selected table for inputted value.
 ...
 * @return bool
 */
function dataExistsOld(string $selectField, string $selectTable, string $whereField, string $whereValue, mysqli $connection): bool
{
...
function getRestaurantTables(mysqli $connection): array
{
    ...
    $data = [];
    $sql = "SELECT id_restaurant_table, number FROM restaurant_tables ORDER BY name ASC";
...
function getWaiters(mysqli $connection): array
{

    $data = [];
    $sql = "SELECT id_waiter,name,status FROM waiters ORDER BY name ASC";
